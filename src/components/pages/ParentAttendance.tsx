...
import { Calendar, Clock, CheckCircle, XCircle, AlertCircle, Download, Filter, TrendingUp } from 'lucide-react';
...
export function ParentAttendance() {
  ...
  const handleDownloadReport = () => {
    const csvContent = [
      ['Date', 'Subject', 'Status', 'Time', 'Reason'],
    ...
    a.href = url;
    a.download = `john_smith_attendance_${new Date().toISOString().split('T')[0]}.csv`;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">John's Attendance</h1>
          <p className="text-gray-600">Monitor your child's attendance record and patterns.</p>
        </div>
        <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-green-50 p-4 rounded-lg">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <h4 className="text-green-900 font-medium">Excellent Attendance</h4>
            </div>
            <p className="text-green-700 text-sm">
              John maintains excellent attendance with a {attendanceRate}% rate. This consistency supports his academic success.
            </p>
          </div>
          <div className="bg-blue-50 p-4 rounded-lg">
            <div className="flex items-center space-x-2 mb-2">
              <Calendar className="h-5 w-5 text-blue-600" />
              <h4 className="text-blue-900 font-medium">Perfect Days</h4>
            </div>
            <p className="text-blue-700 text-sm">
              John has had {Math.floor(presentCount / subjects.length)} perfect attendance days this month.
            </p>
          </div>
        </div>
      </div>
